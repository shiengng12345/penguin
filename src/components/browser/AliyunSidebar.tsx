// Aliyun tab sidebar. Two sections — Accounts (RAM users + optional
...
const ALIYUN_LINK_PREFIX = "aliyun-link-";

export function isAliyunLinkShortcutId(id: string): boolean {
  return id.startsWith(ALIYUN_LINK_PREFIX);
}

// Build the virtual BrowserShortcut feeding the right-pane InlineWebview
// when the user picks an Aliyun link. Account is required: the whole
// point of the tab is one-click logged-in access.
export function aliyunLinkToBrowserShortcut(
  link: AliyunLink,
  ...
  return {
    ...
    // for that account. account.id already carries the "aliyun-acc-"
    // prefix — use it directly to avoid double-prefix in the data dir
    // path (e.g. "aliyun-acc-aliyun-acc-xxx" was wrong).
    dataKey: account.id,
    createdAt: link.createdAt,
  };
}

export interface AliyunSidebarProps {
  ...
  // row UI for Aliyun/Jenkins — the user expects uniform behavior.
  autoSubmitGlobal: boolean;
  autoSubmitByLinkId: Record<string, boolean>;
  // Default ⚡ state: ON when the link has prefill data (which Aliyun
  // links always do — they carry the bound account's username/password).
  // BrowserPage owns the resolver so vault/argo/aliyun/jenkins share
...
export function AliyunSidebar({
  ...
  onDuplicate,
  pendingClearLinkId,
}: AliyunSidebarProps): ReactElement {
  const accounts = useAppStore((s) => s.aliyun.accounts);
  const links = useAppStore((s) => s.aliyun.links);
  const addAccount = useAppStore((s) => s.addAliyunAccount);
  const removeAccount = useAppStore((s) => s.removeAliyunAccount);
  const addLink = useAppStore((s) => s.addAliyunLink);
  const removeLink = useAppStore((s) => s.removeAliyunLink);
